...
pub struct NodeLink<T>(Handle<NodeData<T>>);

impl<T: Debug> Debug for NodeLink<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("NodeLink").field(&self.0).finish()
    }
}

impl<T> Copy for NodeLink<T> {}
...
impl<T> Hash for NodeLink<T> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.0.hash(state);
    }
}

impl<T> Eq for NodeLink<T> {}
impl<T> PartialEq for NodeLink<T> {
...
struct NodeData<T> {
    data: T,
    parent: Option<NodeLink<T>>,
    children: IndexSet<NodeLink<T>>,
}
...
impl<T> NodeTree<T> {
    ...
    pub fn contains(&self, node: NodeLink<T>) -> bool {
        self.nodes.contains(node.0)
    }

    pub fn get(&self, node: NodeLink<T>) -> Option<&T> {
        Some(&self.nodes.get_data(node.0)?.data)
    }

    pub fn get_mut(&mut self, node: NodeLink<T>) -> Option<&mut T> {
        Some(&mut self.nodes.get_data_mut(node.0)?.data)
    }

    pub fn parent_of(&self, node: NodeLink<T>) -> Option<NodeLink<T>> {
        self.nodes.get_data(node.0)?.parent
    }

    pub fn children_of(&self, node: NodeLink<T>) -> Option<impl Iterator<Item = &NodeLink<T>>> {
        let node_children = &self.nodes.get_data(node.0)?.children;
        Some(node_children.iter())
    }

    pub fn insert(&mut self, data: T) -> NodeLink<T> {
        NodeLink(self.nodes.insert(NodeData::new(data)))
    }

    pub fn insert_with_parent(&mut self, data: T, parent: NodeLink<T>) -> NodeLink<T> {
    ...
    pub fn remove(&mut self, node: NodeLink<T>) -> Option<T> {
    ...
    pub fn set_parent(&mut self, child: NodeLink<T>, parent_option: Option<NodeLink<T>>) -> bool {
        ...
        fn resolve_recursive<T>(
            ...
            parent: NodeLink<T>,
            source: NodeLink<T>,
            old_parent_option: Option<NodeLink<T>>,
        ) {
...
mod tests {
    ...
    fn tree_inserts() {
        ...
        assert_eq!(tree.parent_of(leaf1), Some(root));
        assert_eq!(tree.parent_of(stem), Some(root));
        assert_eq!(tree.parent_of(leaf2), Some(stem));
    ...
    fn tree_removals() {
        ...
        assert_eq!(tree.parent_of(root), None);
        assert_eq!(tree.parent_of(leaf1), Some(root));
        assert_eq!(tree.parent_of(leaf2), Some(root));
        assert_eq!(tree.parent_of(leaf3), Some(root));
        ...
        // check if parents were transferred
        assert_eq!(tree.parent_of(leaf1), None);
        assert_eq!(tree.parent_of(leaf2), None);
        assert_eq!(tree.parent_of(leaf3), None);
    ...
    fn re_parent() {
        ...
        assert_eq!(tree.parent_of(root), None);
        assert_eq!(tree.parent_of(leaf1), Some(root));
        assert_eq!(tree.parent_of(stem), Some(leaf1));
        assert_eq!(tree.parent_of(leaf2), Some(stem));
        assert_eq!(tree.parent_of(leaf3), Some(stem));
    ...
    fn recursive_reparent() {
        ...
        assert_eq!(tree.parent_of(leaf1), Some(root));
        assert_eq!(tree.parent_of(leaf3), Some(root));
        assert_eq!(tree.parent_of(leaf4), Some(leaf3));
        assert_eq!(tree.parent_of(stem), Some(leaf3));
        assert_eq!(tree.parent_of(leaf2), Some(stem));
