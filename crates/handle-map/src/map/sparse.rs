...
impl<T> SparseHandleMap<T> {
    ...
    pub fn predict_handle(&self, count: usize) -> Handle<T> {
        match self.open_data.get(count) {
            Some(index) => self.data[*index].handle,
            None => {
                let new_indices = count - self.open_data.len();
                let index = self.data.len() + new_indices;
...
mod tests {
    ...
    fn insert_remove() {
        ...
        assert!(map.get_data(handle).unwrap() == &42);

        let handle2 = map.insert(1234);
        assert!(map.len() == 2);
        assert!(map.contains(handle2));
        assert!(map.get_data(handle2).unwrap() == &1234);
    ...
    fn predict() {
        ...
        assert!(map.get_data(future_handle0).unwrap() == &1234);
        assert!(map.contains(future_handle2));
        assert!(map.get_data(future_handle2).unwrap() == &6789);
    }
}
